...
## 케이스 파일은 공통으로 맞추지 않습니다

**기대값을 사람이 적지 않기 때문입니다.** 정답은 항상 진짜 Redis가 정합니다.
그래서 케이스가 사람마다 달라도 정답이 갈릴 일이 없고, 케이스 파일은
**"내가 어디까지 찔러봤나"**를 나타내는 지표가 됩니다.

- A: `PING`, `PING hello` → 초록불
- B: 거기에 `PING a b`, 소문자 `ping`, 빈 커맨드까지 → 빨간불

둘 다 "difftest 통과"라고 말하지만 의미가 전혀 다릅니다. **이 차이가 세션의 재료입니다.**

세션에서는 서로 케이스를 바꿔 돌려봅니다.

```bash
./scripts/difftest.sh 경로/철수가-쓴-01-ping.txt   # 남의 케이스로 내 서버 돌리기
```

남의 케이스에 내 구현이 깨지면 그게 그대로 발표거리입니다.

**main에는 셋의 합집합을 넣습니다.** 케이스 파일은 줄 추가라 합쳐도 충돌이 없습니다.
당번이 구현 PR을 올릴 때 세 명이 쓴 케이스를 모두 합쳐서 넣습니다.
구현은 당번 것만 남지만, **테스트는 셋 다 남습니다.**
